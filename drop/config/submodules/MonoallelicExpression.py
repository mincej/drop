...
class MAE(Submodule):
    ...
    def checkConfigSampleannotation(self):
        ...
        if len(self.genomeFiles.keys()) > 1:  # more than 1 value in config defined genome dictionary
            if "GENOME" not in subset.columns.values:  # GENOME column not defined
                logger.error(
                    "ERROR: More than 1 genome is defined globally in the config. However the sample matching column "
                    "GENOME is not in the sample annotation column"
                )
                raise KeyError
                ...
                if all(subset["GENOME"] == "nan"):  # GENOME is all empty
                    logger.error(
                        "ERROR: More than 1 genome is defined globally in the config. However the sample matching "
                        "column GENOME is empty"
                    )
                    raise KeyError
                else:  # GENOME is not empty
                    if len(set(subset["GENOME"]) - set(self.genomeFiles.keys())) > 0:  # sa table contains unknown value
                        logger.error(
                            "ERROR: A value (or empty) in the sample annotation table is not defined as a genome key "
                            "in the config"
                        )
                        raise KeyError
            ...
            if "GENOME" not in subset.columns.values:  # GENOME column not defined
                ...
                if list(self.genomeFiles.keys()) == list(
                        self.genomeFiles.values()):  # genome is defined as a string in config
                    if all(subset["GENOME"] == "nan"):  # GENOME is all empty
                        logger.warning(
                            "WARNING: genome is defined as a singular global string instead of a dict. This will be "
                            "deprecated in the future."
                        )
                    else:
                        logger.warning(
                            "WARNING: genome is defined as a singular global string instead of a dict. Information in "
                            "the sample annotation table column GENOME will be ignored."
                        )
                else:  # genome is defined as dict in the config of length 1 and GENOME column exists
                    if all(subset["GENOME"] == "nan"):  # GENOME is all empty
                        pass  # desired behavior
                    else:  # GENOME is not empty
                        if len(set(subset["GENOME"]) - set(
                                self.genomeFiles.keys())) > 0:  # sa table contains unknown values
                            logger.error(
                                "ERROR: A value (or empty) in the sample annotation table is not defined as a genome "
                                "key in the config"
                            )
                            raise KeyError
                        else:
    ...
    def createMaeIDS(self, id_sep='--'):
        ...
        """
        grouped_rna_ids = self.sampleAnnotation.subsetGroups(self.groups, assay="RNA")
        self.checkSubset(grouped_rna_ids, warn=1, error=1)
        id_map = self.sampleAnnotation.idMapping
        mae_ids = {}
